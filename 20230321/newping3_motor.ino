...
long sonar_front(void)  //초음파 센서 1번 측정함수 
{
...
long sonar2(void)  //초음파 센서 2번 측정함수 
...
long sonar3(void)  //초음파 센서 3번 측정함수 
...
void motor_A_control(int direction_a, int motor_speed_a) //모터의 방향(direction)과 속도(speed)제어
{
  if(direction_a == HIGH)
  {
    digitalWrite(IN1,LOW);  //모터의 방향제어
    digitalWrite(IN2,HIGH);
    analogWrite(ENA,motor_speed_a); //모터의 속도제어
  }
  else
  {
    digitalWrite(IN3, LOW);
    digitalWrite(IN4,HIGH);
    analogWrite(ENB,motor_speed_a);
  }
}
void motor_B_control(int direction_b, int motor_speed_b) //모터의 방향(direction)과 속도(speed)제어
{
  if(direction_b == HIGH)
  {
    digitalWrite(IN3, HIGH);  //모터의 방향제어
    digitalWrite(IN4,LOW);
    analogWrite(ENA,motor_speed_b); //모터의 속도제어
  }
  else
  {
    digitalWrite(IN3, LOW);
...

{
  ...
  motor_A_control(HIGH,100);  //오른쪽으로 전진
  motor_B_control(LOW ,100);  //왼쪽으로 전진
